...
def setPluginSetting(name, value, namespace = None):
    ...
    code = _settingValueFromName(namespace, name, "onChange")
    if code is not None:
        oldValue = pluginSetting(name, namespace)
        accept, value = _runFromString(code, oldValue, value)
        if not accept:
            return
...
def pluginSetting(name, namespace=None, typ=None):
    ...
    if QSettings().contains(full_name):
        if typ is None:
            typ = _type_map(_settingValueFromName(namespace, name, 'type'))
        v = QSettings().value(full_name, None, type=typ)
        if isinstance(v, QPyNullVariant):
            v = None
        return v
    else:
        return _settingValueFromName(namespace, name, "default")

_settings = {}

def _settingFromName(namespace, name):
    for setting in _settings[namespace]:
        if setting["name"] == name:
            return setting
    return None

def _settingValueFromName(namespace, name, key):
    for setting in _settings[namespace]:
        if setting["name"] == name:
...
def readSettings(settings_path=None):
    ...
    {"name":"mysetting",
     ...
     "group": "Group 1",
     "onEdit": "print 'Value edited in settings dialog'",
     "onChange": "onchangecode.py"
    },
    {"name":"anothersetting",
    ...
    The onEdit property contains code that will be executed when the user edits the value 
    in the settings dialog. If, after it has been executed, the setting widget should recover its,
    original value, set 'accept = False' in your code.

    The onChange property contains code that will be executed when the setting is changed after
    closing the settings dialog, or programatically by calling the setPluginSetting method.
    If the value is not correct and the setting should not be modified set 'accept = False' in your code.

    Both onEdit and onChange are optional properties. Instead of adding the code directly as the property value,
    you can add a string with a filename (relative to the settings.json file) where the code is stored

    In the code to be run, you will have two variables available:
      - oldValue
      - newValue
    '''
    ...
    def _loadCode(setting, n):
        if n in setting:
            filepath = os.path.join(os.path.dirname(_callerPath()), setting[n])
            if os.path.exists(filepath):
                with open(filepath) as f:
                    setting["onEdit"] = f.read()
    for s in _settings[namespace]:
        _loadCode(s, "onEdit")
        _loadCode(s, "onChange")
...
class TreeSettingItem(QTreeWidgetItem):
    ...
    def __init__(self, parent, tree, setting, namespace, value):
        ...
        self.onEditCode = setting.get("onEdit", None)
        ...
        if self.settingType == CRS:
            def edit():
                ...
                if selector.exec_():
                    ...
                    if authId.upper().startswith("EPSG:"):
                        accept, authId = _runFromString(self.onEditCode, value, authId)
                        if accept:
            ...
            def edit():
                ...
                if f:
                    accept, f = _runFromString(self.onEditCode, value, f)
                    if accept:
                        self.newValue = f
            ...
            self.tree.setItemWidget(self, 1, self.checkbox)                
            def _onEditBool(state):
                accept, state = _runFromString(self.onEditCode, value, state)
                if accept:                    
                    self.checkbox.setCheckStateText(state)
            self.checkbox.stateChanged.connect(_onEditBool)
            ...
            def edit():
                ...
                ret = dlg.exec_()
                if ret:
                    accept, authcfg = _runFromString(self.onEditCode, value, dlg.authcfg)
                    if accept:
                        self.newValue = authcfg
...
def _runFromString(code, oldValue, newValue):
    if code is None:
        return True, newValue

    ns = {}
    ns['oldValue'] = oldValue
    ns['newValue'] = newValue
    ns['accept'] = True

    exec((code), ns)

    return ns['accept'], ns['newValue']